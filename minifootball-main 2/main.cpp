...
void showInGameOnePlayer(int a, int b, int time)
{
	...
	SDL_Color color = {255, 255, 255};
	std::string output = "You \n" + std::to_string(a) + " - " + std::to_string(b) + "\n Computer";
	SDL_Surface *surfaced = TTF_RenderText_Blended(font, output.c_str(), color);
	if (surfaced == nullptr)
	{
		printf("Failed to render text! SDL_ttf Error: %s\n", TTF_GetError());
		TTF_CloseFont(font);
	...
	SDL_Texture *texture = SDL_CreateTextureFromSurface(gRenderer, surfaced);
	if (texture == nullptr)
	{
		printf("Failed to create texture from surface! SDL Error: %s\n", SDL_GetError());
	...
	SDL_Rect dstrect = {140, 300, surfaced->w, surfaced->h};
	SDL_RenderCopy(gRenderer, texture, NULL, &dstrect);
	...
	// Rendering time
	TTF_Font *fontfont = TTF_OpenFont("./asset/fonts/fontTest.ttf", 50);
	if (fontfont == nullptr)
	{
		printf("Failed to load font! SDL_ttf Error: %s\n", TTF_GetError());
		return;
	...
	SDL_Surface *surfaceTime = TTF_RenderText_Blended(fontfont, timeString.c_str(), color);
	if (surfaceTime == nullptr)
	{
		printf("Failed to render text! SDL_ttf Error: %s\n", TTF_GetError());
		TTF_CloseFont(font);
		TTF_CloseFont(fontfont);
		return;
	}

	SDL_Texture *textureTime = SDL_CreateTextureFromSurface(gRenderer, surfaceTime);
	if (textureTime == nullptr)
	{
		printf("Failed to create texture from surface! SDL Error: %s\n", SDL_GetError());
	...
	SDL_Rect dstrectTime = {320, 680, surfaceTime->w, surfaceTime->h};
	SDL_RenderCopy(gRenderer, textureTime, NULL, &dstrectTime);
...
void OnePlayer()
{
	...
	while (game_running)
	{
		...
		{
			// Increase team2Point when the ball reaches the bottom of the screen
			team2Point++;

			// Reset the ball's movement and position for the next round
			if (turn % 2 == 0)
				dy = 5;
			else
				dy = -5;

			// Set a new horizontal movement for the ball
			dx = random_int(3, 5) * (touchNum % 2 == 0 ? 1 : (-1));

			// Reset the ball's position to start a new round
			ballRect = {360, 475, 20, 20};

			// Increment the turn counter for tracking purposes
			turn++;

			// Delay the game for 1000 milliseconds (1 second)
			SDL_Delay(1000);
		}
